...
public class ZigzagBeamPattern : BulletPattern
{
    ...
    {
        return new ZigzagtBeamInstance(this);
    }
}
...
public class ZigzagtBeamInstance : PatternInstance
{
    ...
    public ZigzagtBeamInstance(ZigzagBeamPattern pattern)
    {
    ...
    public override void Fire(Transform firePoint)
    {
        ...
        for (Vector3 position = new Vector3(0, InfoStatic.ScreenBot * 2, 0); position.y < 0; position.y += 1f, position.x -= 1f)
        {
            //좌우에서 위 45도 각도로 발사
        ...
        for (Vector3 position = new Vector3(0, InfoStatic.ScreenBot * 2, 0); position.y < 0; position.y += 1, position.x += 1)
        {
            //좌우에서 위 45도 각도로 발사
            Vector3 dir = Quaternion.Euler(0, 0, 45) * Vector3.up;
            LaserBeam laser = (LaserBeam)BulletManager.Instance.FireBullet(position, dir.normalized, 2, pattern.Speed, pattern.BulletDatas[0].bulletId);
